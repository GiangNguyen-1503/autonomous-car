import time
import sys
import threading
from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer

import cv2
import numpy as np

#--------------
CAM_INDEX = 0
FRAME_W = 320
FRAME_H = 240
DISPLAY_SCALE = 2

AUTO_CALIBRATE = True
CALIB_SECONDS = 3.0

FLOOR_THRESH = 40
FREE_MIN = 0.25
STEER_MARGIN = 0.15
N_COLS = 3

DANGER_LINE_Y = 0.70
DANGER_MIN = 0.2
DANGER_SAFE = 0.40

ACT_DURATION = 0.3
CONFIRM_FRAMES = 3
EXIT_CONFIRM = 4

ENTRY_BACK_S = 0.4
TURN_NUDGE_S = 0.3
WARM_UP_FRAMES = 3

USE_ARDUINO = True
ARDUINO_PORT = '/dev/ttyACM0'
ARDUINO_BAUD = 9600

SAVE_VIDEO = True
OUTPUT_PATH = 'run_output.avi'
STREAM_ENABLE = True
STREAM_PORT = 8000

# === NEW: SCAN MODE ===
SCAN_TURNS = 4
SCAN_TURN_ANGLE = 3.0
SCAN_WAIT_TIME = 0.5

#--------------

THRESHOLDS = {
    'floor': FLOOR_THRESH,
    'free': FREE_MIN,
    'danger': DANGER_MIN,
    'floor_hist': None,
    'poison_threshold': 0.0,
}

CALIB_MIN_RATIO = 0.30
POISON_STD_K = 3.0

#auto calibrate
def calibrate_thresholds(cap, seconds=CALIB_SECONDS):
    print("[calibrate] START CALIBRATING")
    for i in range(3, 0, -1):
        print("[calibrate] %d..." % i)
        time.sleep(1)
    print("[calibrate] measuring...")

    hist_samples = []
    sample_bp_means = []
    ratios_samples = []
    danger_samples = []
    otsu_samples = []

    t_end = time.time() + seconds
    while time.time() < t_end:
        ok, frame = cap.read()
        if not ok or frame is None:
            continue

        h, w = frame.shape[:2]
        hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
        hsv = cv2.bilateralFilter(hsv, 5, 50, 50)
        sx1, sx2 = int(w * 0.30), int(w * 0.70)
        sy1, sy2 = int(h * 0.85), h
        floor = hsv[sy1:sy2, sx1:sx2]
        hist = cv2.calcHist([floor], [0, 1], None, [30, 32], [0, 180, 0, 256])
        cv2.normalize(hist, hist, 0, 255, cv2.NORM_MINMAX)
        hist_samples.append(hist)
        ry1, ry2 = int(h * 0.50), sy1
        region = hsv[ry1:ry2, :]
        bp = cv2.calcBackProject([region], [0, 1], hist, [0, 180, 0, 256], scale=1)
        bp = cv2.medianBlur(bp, 5)

        otsu_thresh, bp_bin = cv2.threshold(bp, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        otsu_samples.append(otsu_thresh)

        col_w = w // N_COLS
        r = []
        for i in range(N_COLS):
            col = bp_bin[:, i * col_w:(i + 1) * col_w]
            r.append((col > 0).sum() / max(col.size, 1))
        ratios_samples.append(r)

        danger_y = max(0, int(h * DANGER_LINE_Y) - ry1)
        if danger_y < bp_bin.shape[0]:
            near = bp_bin[danger_y:, col_w:2 * col_w]
            if near.size > 0:
                danger_samples.append((near > 0).sum() / near.size)

        sample_bp = cv2.calcBackProject([floor], [0, 1], hist, [0, 180, 0, 256], scale=1)
        sample_bp_means.append(float(np.mean(sample_bp)))

    if not otsu_samples:
        print("[calibrate] ERROR: CAN NOT READ FRAME")
        return False

    avg_ratio = float(np.mean([np.mean(r) for r in ratios_samples]))

    if avg_ratio < CALIB_MIN_RATIO:
        print("[calibrate] REJECT: avg_ratio = %.2f < %.2f" % (avg_ratio, CALIB_MIN_RATIO))
        print("[calibrate] -> Recalibrate or move robot")
        return False

    new_floor  = max(15, min(120, int(round(float(np.median(otsu_samples))))))
    new_free = max(0.20, min(0.80, round(avg_ratio * 0.7, 2)))
    new_danger = round(float(np.mean(danger_samples)) * 0.5, 2) if danger_samples else DANGER_MIN
    new_danger = max(0.10, min(0.60, new_danger))

    THRESHOLDS['floor']  = new_floor
    THRESHOLDS['free']   = new_free
    THRESHOLDS['danger'] = new_danger

    avg_hist = np.mean(np.stack(hist_samples), axis=0)
    cv2.normalize(avg_hist, avg_hist, 0, 255, cv2.NORM_MINMAX)
    THRESHOLDS['floor_hist'] = avg_hist

    bp_mean = float(np.mean(sample_bp_means))
    bp_std = float(np.std(sample_bp_means))
    THRESHOLDS['poison_threshold'] = max(0.0, bp_mean - POISON_STD_K * bp_std)

    print("[calibrate] DONE: floor=%d free=%.2f danger=%.2f" % (new_floor, new_free, new_danger))
    print("[calibrate] bp_mean=%.1f std=%.1f" % (bp_mean, bp_std))

    return True

#stream mjeg
class FrameBuffer:
    def __init__(self):
        self._jpeg = None
        self._lock = threading.Lock()

    def update(self, bgr_image):
        ok, buf = cv2.imencode('.jpg', bgr_image, [cv2.IMWRITE_JPEG_QUALITY, 75])
        if ok:
            with self._lock:
                self._jpeg = buf.tobytes()

    def get(self):
        with self._lock:
            return self._jpeg

_frame_buffer = FrameBuffer()

class _MJPEGHandler(BaseHTTPRequestHandler):
    def log_message(self, *args):
        pass

    def do_GET(self):
        self.send_response(200)
        self.send_header('Content-Type', 'multipart/x-mixed-replace; boundary=frame')
        self.end_headers()
        try:
            while True:
                jpeg = _frame_buffer.get()
                if jpeg is None:
                    time.sleep(0.05)
                    continue
                self.wfile.write(b'--frame\r\n')
                self.send_header('Content-Type', 'image/jpeg')
                self.send_header('Content-Length', str(len(jpeg)))
                self.end_headers()
                self.wfile.write(jpeg)
                self.wfile.write(b'\r\n')
                time.sleep(0.1)
        except (BrokenPipeError, ConnectionResetError):
            pass

def start_stream_server(port):
    server = ThreadingHTTPServer(('0.0.0.0', port), _MJPEGHandler)
    threading.Thread(target=server.serve_forever, daemon=True).start()
    return server

#motor
class MotorController:
    def __init__(self, use_arduino, port, baud):
        self.ser = None
        self.last_cmd = None
        if use_arduino:
            try:
                import serial
                self.ser = serial.Serial(port, baud, timeout=1)
                time.sleep(2)
                print("[motor] Arduino CONNECTED AT:", port)
            except Exception as e:
                print("MOTOR cannot connect:", e)
        else:
            print("MOTOR USE_ARDUINO = FALSE")

    def send(self, cmd):
        if cmd == self.last_cmd:
            return

        print("MOTOR ->", cmd)
        self.last_cmd = cmd
        if self.ser is not None:
            try:
                self.ser.write(cmd.encode())
            except Exception as e:
                print("MOTOR SERIAL ERROR:", e)

    def stop(self):
        self.send('x')

    def close(self):
        if self.ser is not None:
            self.stop()
            self.ser.close()


#detection
def detect_floor(frame_bgr):
    h, w = frame_bgr.shape[:2]
    hsv = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2HSV)
    hsv = cv2.bilateralFilter(hsv, 5, 50, 50)

    sx1, sx2 = int(w * 0.30), int(w * 0.70)
    sy1, sy2 = int(h * 0.85), h
    floor = hsv[sy1:sy2, sx1:sx2]

    if THRESHOLDS['floor_hist'] is not None:
        hist = THRESHOLDS['floor_hist']
    else:
        hist = cv2.calcHist([floor], [0, 1], None, [30, 32], [0, 180, 0, 256])
        cv2.normalize(hist, hist, 0, 255, cv2.NORM_MINMAX)

    rx1, rx2 = 0, w
    ry1, ry2 = int(h * 0.50), sy1
    region = hsv[ry1:ry2, rx1:rx2]
    bp = cv2.calcBackProject([region], [0, 1], hist, [0, 180, 0, 256], scale=1)
    bp = cv2.medianBlur(bp, 5)
    
    mean_bp = np.mean(bp)
    
    _, bp_bin = cv2.threshold(bp, THRESHOLDS['floor'], 255, cv2.THRESH_BINARY)

    mask_full = np.zeros((h, w), dtype=np.uint8)
    mask_full[ry1:ry2, rx1:rx2] = bp_bin

    col_w = w // N_COLS
    ratios = []
    for i in range(N_COLS):
        col = bp_bin[:, i * col_w:(i + 1) * col_w]
        ratios.append(round((col > 0).sum() / max(col.size, 1), 3))

    danger_y_full = int(h * DANGER_LINE_Y)
    danger_y_local = max(0, danger_y_full - ry1)
    center_x1 = col_w
    center_x2 = 2 * col_w
    if danger_y_local < bp_bin.shape[0]:
        near_strip = bp_bin[danger_y_local:, center_x1:center_x2]
        if near_strip.size > 0:
            danger_ratio = round((near_strip > 0).sum() / near_strip.size, 3)
        else:
            danger_ratio = 0.0
    else:
        danger_ratio = 1.0

    return {
        'mask': mask_full,
        'ratios': ratios,
        'danger_ratio': danger_ratio,
        'sample_box': (sx1, sy1, sx2, sy2),
        'mean_bp': mean_bp,
    }

# === NEW: SCAN STATE ===
class ScanResult:
    """Lưu kết quả scan 4 hướng"""
    def __init__(self):
        self.results = []  # Danh sách (turn_num, center_ratio)
        self.best_idx = -1
        self.best_center = -1
    
    def add_result(self, turn_num, center_ratio):
        self.results.append((turn_num, center_ratio))
        if center_ratio > self.best_center:
            self.best_center = center_ratio
            self.best_idx = turn_num
    
    def get_best(self):
        return self.best_idx, self.best_center

#fsm 3 state
class AdvancedFSM:
    CRUISE   = "CRUISE"
    SCAN     = "SCAN"
    RETURN   = "RETURN"

    def __init__(self):
        self.state = self.CRUISE
        self.warm_up = 0
        self.danger_streak = 0
        
        # === SCAN STATE ===
        self.scan_result = None
        self.scan_turn_count = 0
        self.scan_turn_step = 0

    def is_danger(self, info):
        return info['danger_ratio'] < THRESHOLDS['danger']

    def is_clear(self, info):
        left, center, right = info['ratios']
        return (center >= THRESHOLDS['free'])

    def step(self, info):
        ratios = info['ratios']
        left, center, right = ratios

        # === CRUISE STATE ===
        if self.state == self.CRUISE:
            if self.warm_up > 0:
                self.warm_up -= 1
                self.danger_streak = 0
                if center >= THRESHOLDS['free']:
                    return ('w', "WARM-UP %d/%d going" % (self.warm_up, WARM_UP_FRAMES), self.state, False)
                return ('x', "WARM-UP %d waiting" % self.warm_up, self.state, False)

            # Không có danger, đi bình thường
            if not self.is_danger(info):
                self.danger_streak = 0
                if center >= THRESHOLDS['free']:
                    return ('w', "CRUISE: center clear (%.2f)" % center, self.state, False)
                if left > right + STEER_MARGIN:
                    return ('a', "CRUISE: nudge left", self.state, False)
                if right > left + STEER_MARGIN:
                    return ('d', "CRUISE: nudge right", self.state, False)
                return ('a', "CRUISE: balanced", self.state, False)
            
            # Có danger
            self.danger_streak += 1
            if self.danger_streak >= CONFIRM_FRAMES:
                # === VÀO SCAN STATE ===
                print("[FSM] CRUISE -> SCAN: Stuck, need to scan 4 directions")
                self.state = self.SCAN
                self.scan_result = ScanResult()
                self.scan_turn_count = 0
                self.scan_turn_step = 0
                return ('x', "ENTER SCAN: stop and prepare", self.state, False)
            
            return ('a', "danger streak %d/%d" % (self.danger_streak, CONFIRM_FRAMES), self.state, False)

        # === SCAN STATE ===
        if self.state == self.SCAN:
            # Step 0: Stop tại chỗ
            if self.scan_turn_step == 0:
                motor_cmd = 'x'
                self.scan_turn_step = 1
                return ('x', "SCAN: stop %d/4" % (self.scan_turn_count + 1), self.state, False)
            
            # Step 1: Quẹo trái
            elif self.scan_turn_step == 1:
                motor_cmd = 'a'
                self.scan_turn_step = 2
                return ('a', "SCAN: turn left %d/4" % (self.scan_turn_count + 1), self.state, True)  # need_wait=True
            
            # Step 2: Dừng để đọc frame
            elif self.scan_turn_step == 2:
                motor_cmd = 'x'
                # Lưu kết quả center ratio
                center_ratio = ratios[1]  # CENTER column
                self.scan_result.add_result(self.scan_turn_count, center_ratio)
                print("[SCAN] Turn %d: center=%.2f (best: %d=%.2f)" % 
                      (self.scan_turn_count, center_ratio, self.scan_result.best_idx, self.scan_result.best_center))
                
                self.scan_turn_count += 1
                self.scan_turn_step = 0
                
                # Đã quẹo 4 lần?
                if self.scan_turn_count >= SCAN_TURNS:
                    # === VÀO RETURN STATE ===
                    best_idx, best_center = self.scan_result.get_best()
                    print("[FSM] SCAN done. Best: turn %d (%.2f) | RETURN to face it" % (best_idx, best_center))
                    self.state = self.RETURN
                    self.return_turn_count = 0
                    return ('x', "SCAN DONE: best=%d (%.2f), prepare RETURN" % (best_idx, best_center), self.state, False)
                
                return ('x', "SCAN: read frame %d/4 (C=%.2f)" % (self.scan_turn_count, center_ratio), self.state, False)

        # === RETURN STATE ===
        if self.state == self.RETURN:
            best_idx, best_center = self.scan_result.get_best()
            # Cần quẹo trở lại bao nhiêu lần?
            # best_idx=0 → 0 lần, best_idx=1 → 3 lần (quẹo ngược), v.v.
            need_return_turns = (SCAN_TURNS - best_idx) % SCAN_TURNS
            
            if self.return_turn_count < need_return_turns:
                self.return_turn_count += 1
                return ('a', "RETURN: turn left %d/%d to face best direction" % (self.return_turn_count, need_return_turns), self.state, True)
            
            # Đã quẹo về đúng hướng
            print("[FSM] RETURN complete, back to CRUISE")
            self.state = self.CRUISE
            self.warm_up = WARM_UP_FRAMES
            self.danger_streak = 0
            return ('w', "RETURN DONE: go forward", self.state, False)


#show up
def build_view(frame_bgr, info, cmd, reason, state, fps, scan_result=None):
    h, w = frame_bgr.shape[:2]
    original = frame_bgr.copy()
    result = frame_bgr.copy()

    mask = info['mask']
    green_overlay = np.zeros_like(result)
    green_overlay[mask > 0] = (0, 200, 0)
    result = cv2.addWeighted(result, 1.0, green_overlay, 0.45, 0)

    sx1, sy1, sx2, sy2 = info['sample_box']
    cv2.rectangle(result, (sx1, sy1), (sx2, sy2), (0, 255, 255), 1)
    cv2.putText(result, "Floor color", (sx1 + 2, sy1 - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (0, 255, 255), 1)

    ratios = info['ratios']
    col_w = w // N_COLS
    for i in range(N_COLS):
        x0 = i * col_w
        cv2.line(result, (x0, 0), (x0, h), (255, 200, 0), 1)
        free = ratios[i] >= THRESHOLDS['free']
        color = (0, 220, 0) if free else (0, 0, 255)
        cv2.putText(result, "%.2f" % ratios[i], (x0 + 6, h - 14), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)

    dy = int(h * DANGER_LINE_Y)
    danger_triggered = info['danger_ratio'] < THRESHOLDS['danger']
    line_color = (0, 0, 255) if danger_triggered else (0, 200, 255)
    cv2.line(result, (0, dy), (w, dy), line_color, 2)
    cv2.putText(result, "DANGER (%.2f)" % info['danger_ratio'], (w - 130, dy - 6), cv2.FONT_HERSHEY_SIMPLEX, 0.45, line_color, 1)

    _label(original, "VIDEO")
    _label(result, "RESULT")

    info_h = 130
    canvas = np.zeros((h + info_h, w * 2, 3), dtype=np.uint8)
    canvas[0:h, 0:w] = original
    canvas[0:h, w:w * 2] = result

    state_color_map = {
        "CRUISE": (0, 220, 0),
        "SCAN": (255, 165, 0),
        "RETURN": (255, 100, 255)
    }
    state_color = state_color_map.get(state, (255, 255, 255))
    cmd_color = {'w': (0, 220, 0), 'x': (0, 0, 255), 'a': (0, 200, 255), 'd': (0, 200, 255), 's': (200, 200, 0)}.get(cmd, (255, 255, 255))

    cv2.putText(canvas, "STATE: %s" % state, (12, h + 28), cv2.FONT_HERSHEY_SIMPLEX, 0.7, state_color, 2)
    cv2.putText(canvas, "CMD: %s" % cmd, (280, h + 28), cv2.FONT_HERSHEY_SIMPLEX, 0.7, cmd_color, 2)
    cv2.putText(canvas, reason, (12, h + 56), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (220, 220, 220), 1)
    cv2.putText(canvas, "FPS: %.1f | bp_mean: %.1f" % (fps, info['mean_bp']), (w * 2 - 220, h + 28), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (180, 180, 180), 1)
    
    # === SCAN RESULTS ===
    if scan_result and scan_result.results:
        scan_text = "SCAN: "
        for turn_num, center in scan_result.results:
            marker = "*" if turn_num == scan_result.best_idx else ""
            scan_text += "[%d:%.2f%s] " % (turn_num, center, marker)
        cv2.putText(canvas, scan_text, (12, h + 86), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (100, 200, 255), 1)
    
    cv2.putText(canvas, "Advanced FSM: CRUISE -> SCAN(4 turns) -> RETURN -> CRUISE", (12, h + 106), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (140, 140, 140), 1)

    if DISPLAY_SCALE != 1:
        canvas = cv2.resize(canvas, (canvas.shape[1] * DISPLAY_SCALE, canvas.shape[0] * DISPLAY_SCALE), interpolation=cv2.INTER_LINEAR)

    return canvas

def _label(img, text):
    cv2.rectangle(img, (0, 0), (img.shape[1], 22), (40, 40, 40), -1)
    cv2.putText(img, text, (8, 16), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

#main loop
def main():
    use_auto_calibrate = AUTO_CALIBRATE
    if '--manual' in sys.argv:
        use_auto_calibrate = False

    cap = cv2.VideoCapture(CAM_INDEX, cv2.CAP_V4L2)
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, FRAME_W)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, FRAME_H)
    if not cap.isOpened():
        print("[ERROR] CANNOT OPEN camera index", CAM_INDEX)
        sys.exit(1)

    if AUTO_CALIBRATE and '--manual' not in sys.argv:
        if not calibrate_thresholds(cap):
            print("[ERROR] Calibration failed")
    else:
        print("[run] MANUAL: floor=%d free=%.2f danger=%.2f" % (THRESHOLDS['floor'], THRESHOLDS['free'], THRESHOLDS['danger']))

    motor = MotorController(USE_ARDUINO, ARDUINO_PORT, ARDUINO_BAUD)
    fsm = AdvancedFSM()

    writer = None
    if SAVE_VIDEO:
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        writer = cv2.VideoWriter(OUTPUT_PATH, fourcc, 10.0, (FRAME_W * 2, FRAME_H + 130))

    if STREAM_ENABLE:
        start_stream_server(STREAM_PORT)
        print("[stream] http://<ip-address>:%d" % STREAM_PORT)

    print("[run] START - Advanced FSM with SCAN mode")
    print("[info] When stuck: CRUISE -> SCAN(4 directions) -> RETURN -> CRUISE")

    try:
        frame_count = 0
        while True:
            ok, frame = cap.read()
            if not ok or frame is None:
                time.sleep(0.05)
                continue

            t0 = time.time()
            info = detect_floor(frame)
            result = fsm.step(info)
            cmd, reason, state, need_wait = result
            proc_time = time.time() - t0
            fps = 1.0 / max(proc_time, 0.001)

            view = build_view(frame, info, cmd, reason, state, fps, fsm.scan_result if state == "SCAN" or state == "RETURN" else None)
            if STREAM_ENABLE:
                _frame_buffer.update(view)
            if writer is not None:
                writer.write(view)

            motor.send(cmd)
            
            # === Timing ===
            if cmd == 's':
                time.sleep(ENTRY_BACK_S)
            elif cmd == 'a' and need_wait:
                # SCAN turn: quẹo + chờ để đọc frame
                time.sleep(SCAN_TURN_ANGLE)
                motor.send('x')  # Stop
                time.sleep(SCAN_WAIT_TIME)  # Dừng để đọc
            else:
                time.sleep(ACT_DURATION)

            frame_count += 1
            if frame_count % 100 == 0:
                print("[info] frames=%d fps=%.1f state=%s" % (frame_count, fps, state))

    except KeyboardInterrupt:
        print("\n[RUN] Stop")
    finally:
        motor.close()
        cap.release()
        if writer is not None:
            writer.release()
            print("[RUN] Video saved:", OUTPUT_PATH)
        print("[RUN] FINISH")

if __name__ == "__main__":
    main()
