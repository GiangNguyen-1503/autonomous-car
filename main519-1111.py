...
STUCK_BACK_S   = 4.0   # lui 4s
STUCK_TURN_S   = None  # re trai toi khi thay thoang duong (khong timeout)
...
class FreeSpaceFSM:
    ...
    def step(self, info, roi_h, stuck_flag):
        ...
        if self.state == self.STUCK:
            # Phase waiting: đếm 0-10s từ lúc phát hiện stuck
            if self.stuck_phase is None:
                # Kiểm tra đã đợi 10s chưa
                wait_time = time.time() - self.stuck_detected_time
                if wait_time < 10.0:
                    # Còn đang đếm 0-10s, hiển thị countdown
                    return ('x', "STUCK detected: waiting %.1f/10s before backing up" % wait_time, self.state)
                else:
                    # Đã đợi đủ 10s, bắt đầu lùi
...
def build_view(frame_bgr, info, cmd, reason, state, fps, stuck_flag, stuck_detected_time=None):
    ...
    # Hien thi STUCK countdown: chi hien khi dang trong waiting phase (0-10s)
    stuck_countdown_txt = ""
    if stuck_detected_time is not None and state == "STUCK":
        elapsed = time.time() - stuck_detected_time
        if elapsed <= 10.0:
            stuck_countdown_txt = "STUCK waiting: %.1f/10s" % elapsed
    
    cv2.putText(canvas, "FPS: %.1f  %s" % (fps, stuck_countdown_txt),
                (w * 2 - 300, h + 28),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                (0, 100, 255) if stuck_countdown_txt else (180, 180, 180), 1)
