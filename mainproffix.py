...
# --- Free-space decision ---
BLOCK_RATIO     = 0.30 #absolute floor: c_fwd below this fraction of ROI height is blocked
DANGER_STD_K    = 1.0 #adaptive: also blocked if c_fwd is K std below the frame mean
...
STUCK_OSC_BAND = 5.0 #any region staying within +-10px the whole window
                      #counts as stuck (don't require all three)
STUCK_BACK_S   = 2.5
...
def scan_edges(frame_bgr):
    ...
    #frame-wide stats for the adaptive threshold (scene-relative danger)
    all_ys = [p[1] for p in edge_array]
    edge_mean = float(np.mean(all_ys)) if all_ys else 0.0
    edge_std  = float(np.std(all_ys))  if all_ys else 0.0
...
class FreeSpaceFSM:
    ...
    def _blocked_front(self, info, roi_h):
        #absolute floor catches a uniformly close obstacle/wall;
        #relative check flags the center being notably worse than the scene
        near = info['c_fwd'] < BLOCK_RATIO * roi_h
        relative = info['c_fwd'] < info['edge_mean'] - DANGER_STD_K * info['edge_std']
        return near or relative
...
def build_view(frame_bgr, info, cmd, reason, state, fps, stuck_flag, stuck_detected_time=None):
    ...
    #show both thresholds so they can be compared/tuned during testing
    abs_thr = BLOCK_RATIO * info['roi_h']
    rel_thr = info['edge_mean'] - DANGER_STD_K * info['edge_std']
    cv2.putText(canvas, "block: abs=%.0f rel=%.0f (F=%.0f)" % (abs_thr, rel_thr, info['c_fwd']),
                (w + 12, h + 80),
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (140, 140, 140), 1)
